...
class GoToLabelModule
{
    ...
    public function getRcml($input = null)
    {
        ...
        foreach ($moduleInstanceSettings as $module_instance_setting)
        {
            ...
            {
                case 'moduleId':
                    $workflowUrl = $this->container->getParameter('application_url').'/restcomm/'.$this->moduleInstance->getWorkflow()->getWorkflowId().'/rcml?stepId='.$this->moduleInstance->getWorkflowModuleInstanceId().'&rand='.rand(10000,999999);
