 ...
 */
class ModuleInstanceRepository extends EntityRepository
{
    /**
     *
     */
    public function getNextAdapters($adapterId, $currentStep = null)
    {
        $q = $this->createQueryBuilder('mi')
            ->where('mi.adapter = :adapterId');
        if (!empty($currentStep)) {
            $q = $q->andwhere('mi.moduleInstanceId > :currentStep');
        }
        $q = $q->orderBy('mi.executeOrder', 'ASC')
            ->setParameter('adapterId', $adapterId);
        if (!empty($currentStep)) {
            $q = $q->setParameter('currentStep', $currentStep);
        }
        $q = $q->getQuery();
        return $q->getResult();
